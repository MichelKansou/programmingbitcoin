...
class Tx:
    ...
    def sig_hash(self, input_index):
        ...
        sig += encode_varint(len(self.tx_ins))
        ...
        for tx_out in self.tx_outs:
            sig += TxOut(tx_out.amount, tx_out.script_pubkey).serialize()
        # add the locktime using int_to_little_endian in 4 bytes
        sig += int_to_little_endian(self.locktime, 4)
        # add SIGHASH_ALL using int_to_little_endian in 4 bytes
        sig += int_to_little_endian(1, 4)
        # hash256 the serialization
        sig_hash = hash256(sig)
        # convert the result to an integer using int.from_bytes(x, 'big')
        return int.from_bytes(sig_hash, 'big')
    ...
    def verify_input(self, input_index):
        ...
        combine_script = tx_in.script_sig + script_pubkey
        # evaluate the combined script
        return combine_script.evaluate(z)
    ...
    def sign_input(self, input_index, private_key):
        ...
        # calculate the public key in sec format
